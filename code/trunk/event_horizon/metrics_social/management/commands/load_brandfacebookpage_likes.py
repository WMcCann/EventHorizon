...
class Command(BaseCommand):
    ...
    help = 'Load likes from all posts'
            ...
            for bfp in [b for b in bfps]:
                ...
                while loop:
                    ...
                    for fbmsg in [f for f in fbmsgs]:
                        # get likes
                        path = u'/%s/likes' % (fbmsg.facebook_id)

                        # get pagination
                        try:
                            after = APIPagination.objects.get(
                                app_name=u'metrics_social',
                                model_name=u'FacebookLike',
                                path=path,
                                ).offset
                    ...
                    for task in client.tasks:
                        if u'response' in task:
                            ...
                            page_save = PageSave('metrics_social', u'FacebookLike')
                            page_save.start_save(task[u'response'][u'data'], fbmowner=owner)

                            owner.likes = FacebookLike.objects.filter(message=owner, deleted=False).count()
                            owner.save()

                            if len(task[u'response'][u'data']) < int(task[u'kwargs'][u'limit']):
                                owner.loaded_likes = True
                                owner.likes = FacebookLike.objects.filter(message=owner, deleted=False).count()
                                owner.save()
                                continue
                    ...
                    self.stdout.write(u'Facebook like block was created\n')