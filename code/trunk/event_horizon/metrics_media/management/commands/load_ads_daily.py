...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        accs = [a for a in AdwordsAccount.objects.filter(first_load_ad=True, deleted=False)]
        for acc in accs:
            if acc:
                ...
                filename = os.path.join(settings.PROJECT_ROOT_PATH, 'public', 'media', 'adwords', '%s_ad_%s.csv' % (acc.client_id, until.strftime('%Y-%m-%d')))
